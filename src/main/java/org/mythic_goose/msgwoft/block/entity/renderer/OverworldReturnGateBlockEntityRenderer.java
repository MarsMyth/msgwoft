...
public class OverworldReturnGateBlockEntityRenderer
        implements BlockEntityRenderer<OverworldReturnGateBlockEntity> {

    private static final int SEGMENTS = 64;
    private static final float RADIUS = 0.96f;

    public OverworldReturnGateBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {}

    @Override
    public void render(OverworldReturnGateBlockEntity be, float partialTick,
        ...
        float scale = 1.0f + 0.06f * (float) Math.sin(time * 0.1f);
        poseStack.scale(scale, scale, scale);


        renderDisc(poseStack, buffers.getBuffer(ModRenderTypes.WARPGATE_RETURN));
    ...
    @Override
    public boolean shouldRenderOffScreen(OverworldReturnGateBlockEntity be) {
        return true;
    }
}