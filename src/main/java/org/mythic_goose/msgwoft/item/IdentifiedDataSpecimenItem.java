...
public class IdentifiedDataSpecimenItem extends Item {

    private static final Random RANDOM = new Random();

    public IdentifiedDataSpecimenItem(Properties properties) {
        super(properties);
    }

    public static ItemStack createFromSpecimen(ItemStack source, float con, float cry, float qua) {
        ItemStack stack = new ItemStack(ModItems.IDENTIFIED_DATA_SPECIMEN);

        CompoundTag sourceTag = source.get(DataComponents.CUSTOM_DATA) != null
                ? source.get(DataComponents.CUSTOM_DATA).copyTag()
                : new CompoundTag();
        ...
        // Preserve source block, design and color from original item
        if (sourceTag.contains("SourceBlock")) tag.putString("SourceBlock", sourceTag.getString("SourceBlock"));
        tag.putInt("Design", sourceTag.contains("Design") ? sourceTag.getInt("Design") : RANDOM.nextInt(3));
        tag.putInt("Color",  sourceTag.contains("Color")  ? sourceTag.getInt("Color")  : randomColor());
    ...
    public void appendHoverText(ItemStack stack, TooltipContext ctx, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (Screen.hasAltDown()) {
            if (tag.contains("Color")) {
                String hex = String.format("#%06X", tag.getInt("Color") & 0xFFFFFF);
                tooltip.add(Component.literal("§7Tint: §f" + hex));
            }
            if (tag.contains("Design")) {
                if (tag.getInt("Design") == 0) {
                    tooltip.add(Component.literal("§7Design: §fLayered"));
                } else if (tag.getInt("Design") == 1) {
                    tooltip.add(Component.literal("§7Design: §fMixed"));
                } else if (tag.getInt("Design") == 2) {
                    tooltip.add(Component.literal("§7Design: §fThick"));
                }
            }
        } else {
            tooltip.add(Component.literal("§9Hold 'alt' to see extra info"));
        }
    }
